...
function orderFormClass()
{
	 ...
	 */
	this.addressInit = function(defaultAddressId,provice_id)
	{
		if(defaultAddressId)
		{
			...
			$('input:radio[name="radio_address"][value="'+defaultAddressId+'"]').trigger('click');
                        ...
                        //由于上述代码trigger(click)导致的addressSelected 函数执行时间不能确定  因此 $('[name="province"]').val 结果未知  这里强制给$('[name="province"]').val赋值
                        //$('[name="province"]').val(provice_id);
                        $('[name="province"]').html('<option  value="'+provice_id+'" selected="selected">省</option>');
			this.addressSave();
		}
		else
		{
	...
	this.addressSave = function()
	{
		...
		//获取配送数据并且开启配送方式
		get_delivery($('[name="province"]').val());//页面接口
	...
	this.deliverySave = function()
	{
		...
		//保存运费
		this.deliveryPrice = $('input:radio[name="delivery_id"]:checked').attr('alt');
	...
	this.isSubmit = function()
	{
		...
		{
                        var first = saveButtonList.first();
			first.trigger('focus');
                        
                        first.next().css('display','inline-block');
                        $(document).scrollTop(first.offset().top-400);
                        
                        
                        
                        //$('#'saveButtonList.first().attr('id').replace('button', 'error') )
			return false;
		}
		return true;
	}
