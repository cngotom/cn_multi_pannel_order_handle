	...
	*/
	public function del($goods_id)
	{
		...
		$tb_goods ->del('id='.$goods_id);
                             ...
                             //删除礼册产品
                             $tb_volume_goods = new IModel('volumes_goods');
                             $tb_volume_goods->del('goods_id='.$goods_id);
	}
	/**
	 * 修改商品
	 */
	public function edit($goods_info)
	{
		...
		$goodsPrice   = $groupPriceDB->query("goods_id = ".$goods_info['id']." and product_id = '' ");
		$temp = array();
		...
		if($cateData)
		{
			foreach($cateData as $item)
			{
				$data['goods_category'][] = $item['category_id'];
                            ...
                            //礼册分类
		$volumeExtend = new IQuery('volumes_goods');
		$volumeExtend->where = 'goods_id = '.$goods_info['id'];
		$volumeExtend->fields = 'volume_id';
		$volumeData = $volumeExtend->find();
		if($volumeData)
		{
			foreach($volumeData as $item)
			{
				$data['goods_volumes'][] = $item['volume_id'];
			}
		}
		return $data;
	}
