...
<div class="pop_win">
	<form action='<?php echo IUrl::creatUrl("/goods/spec_update");?>' method='post' id='specForm'>
		<table class="form_table" style="width:90%">
			...
			<tr>
				<td style='text-align:right'>规格名称：</td><td><input class="normal" name="name" value="<?php echo isset($name)?$name:"";?>" type="text" pattern="required" alt="名字不能为空" /><label>* 规格名称（必填）</label></td>
			</tr>
			<tr>
				<td style='text-align:right'>显示类型：</td><td><label class="attr"><input name="type" type="radio" value="1" <?php if($type==1 || $type==null){?>checked=checked<?php }?> /> 文字</label><label class="attr"><input name="type" type="radio" value="2" <?php if($type==2){?>checked=checked<?php }?> /> 图片</label></td>
			</tr>
			<tr>
				<td style='text-align:right'>显示备注：</td><td><input class="normal" type="text" name="note" value="<?php echo isset($note)?$note:"";?>" /></td>
			</tr>
			<tr>
						...
						<tbody id='spec_box'>
							...
							<tr class='td_c'>
								<td>
									...
									<?php if($type==2){?><div class="imgbox"><img class="img_border" src='<?php echo IUrl::creatUrl("")."$item";?>' width='50px' height='50px' /></div><input type="hidden" name="value[]" value="<?php echo isset($item)?$item:"";?>" /><button type="button" class="btn"><span>选择图片</span></button><?php }?>
								</td>
								<td>
