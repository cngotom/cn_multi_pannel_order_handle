...
<link rel="stylesheet" href="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/css/help_center.css";?>" />


        <!--位置 star-->
        <div class="theme">
            <img src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/index/gif_08.jpg";?>">您所在的位置： <a href="<?php echo IUrl::creatUrl("");?>"> 首页</a> &gt; <a href="<?php echo IUrl::creatUrl("/site/help_list/id/$cat_id");?>"><?php echo isset($this->cat_row['name'])?$this->cat_row['name']:"";?></a>
        </div>
        <!--位置 end-->
        <div class="gift_theme">
            <!--帮助中心左边 star-->
            <div class="giftboos_left">
            <div class="gL_up">
                    <img src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/index/help_L_top.jpg";?>"></div>
                <div class="gL_middle">
                <?php $query = new IQuery("help_category");$query->where = "position_left = 1";$query->order = "sort asc,id desc";$items = $query->find(); foreach($items as $key => $item){?>
                <dl><dt><?php echo isset($item['name'])?$item['name']:"";?></dt> 
			<?php $query = new IQuery("help");$query->where = "cat_id = $item[id]";$query->order = "sort desc,id desc";$items = $query->find(); foreach($items as $key => $help_row){?>
                         <dd><a href="<?php echo IUrl::creatUrl("/site/help/id/$help_row[id]/");?>" title="<?php echo isset($help_row['name'])?$help_row['name']:"";?>"><?php echo isset($help_row['name'])?$help_row['name']:"";?></a></dd>
			<?php }?>
		</dl>
		<?php }?>
            </div>
                <div class="gL_down">
                    <img src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/index/gift_L_bottom.jpg";?>"></div>
        </div>
            <!--帮助中心左边 end-->
            <!--帮助中心右边边 star-->
