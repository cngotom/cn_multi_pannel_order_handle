...
<div class="ucenter container">
    ...
    <table class="list_table m_10" width="100%" cellpadding="0" cellspacing="0">
		...
		<tbody>
		<?php $msg =  new Mess($user['user_id']); $ids = $msg->get_mess_ids();$ids = substr($ids,-1,1)==','?substr($ids,0,-1):$ids?>
        <?php $page= (isset($_GET['page'])&&(intval($_GET['page'])>0))?intval($_GET['page']):1;?>
        <?php if($ids === null || $ids == ''){?><?php $ids=0?><?php }?>
            <?php $query = new IQuery("message");$query->where = "id in($ids)";$query->page = "$page";$query->order = "id desc";$items = $query->find(); foreach($items as $key => $item){?>
            	<tr>
                	<td class="t_l"><label class="blue"><?php echo isset($item['title'])?$item['title']:"";?></label></td><td><?php echo isset($item['time'])?$item['time']:"";?></td><td id="msg_id_<?php echo isset($item['id'])?$item['id']:"";?>"><?php if($msg->is_read($item['id'])){?>已读<?php }else{?>未读<?php }?></td><td class="blue"><a class='blue' href="<?php echo IUrl::creatUrl("/ucenter/message_del/id/$item[id]");?>">删除</a><a class="blue" href="javascript:void(0)" onclick="show_msg(this,'<?php echo isset($item['content'])?$item['content']:"";?>',<?php echo isset($item['id'])?$item['id']:"";?>)">查看</a></td>
                </tr>
			<?php }?>
