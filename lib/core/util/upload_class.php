        ...
        foreach($_FILES as $field => $file)
        {
            ...
            if(is_array($_FILES[$field]['name']))
            {
                ...
                foreach($keys as $key)
                {
	                ...
	                //图片木马检测
	                if(in_array($fileext,$this->checkType))
	                {
	                	if(!IFilter::checkHex($_FILES[$field]['tmp_name'][$key]))
	                	{
	                		$fileInfo[$key]['name'] = $_FILES[$field]['name'];
	                  	  	$fileInfo[$key]['flag'] = -3;
	                  	  	continue;
	                	}
	                }
            ...
            else
            {
                ...
                if(in_array($fileext,$this->checkType))
                {
                	if(!IFilter::checkHex($_FILES[$field]['tmp_name']))
                	{
