...
class Simple extends IController
{
        ...
        if($areaArray)
        {
            ...
            foreach($addressList as $key => $val)
            {
                    ...
                    if($val['default'] == 1)
                    {
                            ...
                            $this->defaultPrpvinceId = $val['province'];
                    }
            }
        }
    ...
    {
              $no      = IFilter::act(IReq::get('card_no','post'));
    	$password   = IFilter::act(IReq::get('password','post'));
    	$captcha    = IFilter::act(IReq::get('captcha','post'));
               $ex_goods_id    = IFilter::act(IReq::get('ex_goods_id','post'),'int');
    	...
    	{
                    $message = Giftcard::card_login($no,$password);
                    if(empty($message))
                    {
                        //从兑换产品跳转过来的直接转向产品兑换
                        if($ex_goods_id>0)
                        {
                            //echo $ex_goods_id;exit();
                            $this->redirect("card_do_exchange/ex_goods_id/$ex_goods_id");    
                        }
                        else{
                            $volumes_id = ISafe::get(Giftcard::SESSION_CARD_VOLUME_ID_KEY);
                            //直接进入礼卡兑换的直接进入相关礼册列表
                            $this->redirect("volumes/id/$volumes_id");    
                        }
                    }
          }
         	//出错信息展示
    ...
    function exchange2()
    {
            $login_card_info = Giftcard::get_login_card_info();
            $volume_id =$login_card_info['volume_id'];
            $card_id = $login_card_info['card_id'];
            $goods_id = $login_card_info['goods_id'];
            
            //礼卡未登录 或者未选择礼品 跳转至兑换中心
            if( !isset($volume_id)  || !isset($goods_id)     )
                $this->redirect('exchange');
            else
            {
               //获取产品信息
                $goods_obj = new IQuery('goods as g');
                $goods_obj->join = 'left join volumes as vol on vol.id='.$volume_id;
                $goods_obj->fields = 'g.small_img as small_img,g.id as id,vol.title as title ,g.name as name';
                $goods_obj->where = 'g.id='.$goods_id;
                $goods_list = $goods_obj->find();
    ...
    function exchange3()
    {
            ...
            if( !isset($volume_id)  || !isset($goods_id))
                    $this->redirect('exchange');
            else if (Giftcard::is_card_used())
            {
                Giftcard::destroy();
                IError::show(403,'该礼卡已经被使用');
            }
	...
	//防止表单重复提交
        ...
        /*将订单中的商品插入到order_goods表*/
           ...
           $orderInstance->insertOrderGoods($this->order_id,$goodsResult);
            ...
            //使用礼卡
             Giftcard::do_use_card($this->order_id);           
             Giftcard::destroy();
            //订单自动完成
            $order_id = Order_Class::updateOrderStatus($dataArray['order_no']);
            if($order_id != '')
    ...
    //礼卡兑换产品
    function card_do_exchange()
    {
         $ex_goods_id      = IFilter::act(IReq::get('ex_goods_id'),'int');
         $message = '';
       
         if( !Giftcard::is_card_login())
         {
             $this->redirect("exchange/ex_goods_id/$ex_goods_id");
         }
         else
         {
             $volume_id = ISafe::get('gift_card_volume_id');
             
             //查询改礼卡对应的产品
             $obj_volume_goods = new IModel('volumes_goods');
             $goods_row = $obj_volume_goods->query('volume_id = ' .$volume_id );
             $goods_array = array();
             foreach($goods_row as $row)
             {
                 $goods_array[] = $row['goods_id'];
             }
             print_r($goods_array);
             if( in_array($ex_goods_id,$goods_array))
             {
                 $volume_id = ISafe::set('gift_card_goods_id',$ex_goods_id);
               
                 $this->redirect('/simple/exchange2');
     ...
     function gift_goods()
    {
        ...
        //获取礼册名称
        $volumes_goods = new IQuery('volumes_goods as vg');
        $volumes_goods->join ='left join volumes as v on vg.volume_id = v.id  ';
        $volumes_goods->where = ' vg.goods_id =  '.$goods_id;
        $volumes_goods->fields = 'v.title as title,v.id as id,v.price as price';
        $volumes_info = $volumes_goods->find();
        $goods_info['volumes_info']  = $volumes_info[0];
        ...
        $goods_info['category'] =$categoryRow;
    ...
    {
        $this->layout = 'site';
        $volumes_id = IFilter::act(IReq::get('id'),'int');
        if(!$volumes_id)
        {
                IError::show(403,"传递的参数不正确");
                exit;
       ...
       //使用礼册id获得商品信息
        $tb_volumes = new IModel('volumes');
        $volumes_info = $tb_volumes->getObj('id='.$volumes_id);
        if(!$volumes_info)
        {
                IError::show(403,"礼册不存在");
                exit;
        }
        
        //获取礼册对应的产品
        $volumes = new IQuery('volumes as v');
        $volumes->join = 'right join volumes_goods as vg on vg.volume_id = v.id  right join goods as g on vg.goods_id = g.id ';
        $volumes->where = ' v.id  =  '.$volumes_id;
        $volumes->fields = 'v.title as title,v.id as id,v.price as price,g.name as gname ,g.id as gid,g.list_img as limg';
        $volumes_goods_info = $volumes->find();
        
        $volumes_info['volumes_goods_info']  = $volumes_goods_info;
        
       # print_r($volumes_info);exit();
        $this->setRenderData($volumes_info);
        $this->redirect('volumes');
    }
    ...
    {
        $this->layout = 'site';
        $festival_id = IFilter::act(IReq::get('id'),'int');
        $volume_price = IFilter::act(IReq::get('price'),'int');
        //u
        $festival_info = array();
       //使用节日id获取对应礼册列表]
        ...
        $where = ' 1=1 ';
        if($festival_id)
            $where .= 'and v.festival_id = '.$festival_id;
        if($volume_price)
            $where .= 'and v.price = '.$volume_price;
        $volumes->where = $where;
        
        $volumes->order = 'v.sort asc';
        $volumes->page = $page;
        $volumes->pagesize = 10;
        $volumes_info = $volumes->find();
        $festival_info['pageHtml'] = $volumes->getPageBar();
      
        $festival_info['volumes_list'] = $volumes_info;
        ...
        // 获取所有节日
        $tb_festival = new IModel('festival');
        $festival_info['festival_list'] = $tb_festival->query('visibility = 1','*','`sort`','asc');
         //获取金额段
        $tb_volumes = new IModel('volumes');
        $festival_info['price_list'] =   $tb_volumes->query('','distinct price','price','asc');
   
        
        $this->setRenderData($festival_info);
        $this->redirect('festival');
        
        //print_r($festival_info);
    }
