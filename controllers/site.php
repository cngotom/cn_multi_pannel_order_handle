...
class Site extends IController
{
	...
	function init()
	{
		...
		$checkObj->checkUserRights();
                //缓存
                $this->tempRegister = array(
                    'index' => array( 'cache_id' =>sprintf('%X', crc32('UNIQE'))  ,'lifetime' => 3600 * 24 * 10  )
                );
	}
        function test()
        {
            echo $this->fetch('widget/test.html',array('welcome'=>'Hello'));
            exit();
            $this->layout='inner,outer,ucenter_1';
            $this->redirect('test');
        }
	function index()
	{
	...
	function goodsListFilter()
	{
		...
		$show_type = IFilter::act(IReq::get('show_type'));
	...
	function find($defaultWhere,&$goodsObj)
	{
		...
		if(!empty($specArray))
		{
			foreach($specArray as $key => $val)
			{
				if($key != '' && $val != '')
				{
					$specCond[] = ' spec_id = '.$key.' and spec_value = "'.$val.'"';
				}
			}
		}
	...
	function products()
	{
		...
		$comment_info = $tb_comment->getObj('goods_id='.$goods_id.' and status=1',' sum(point) as point ,count(id) as numbers ');
		$goods_info['comment_point']= 0;
		$goods_info['comment_num']  = 0;
		if($comment_info)
		{
			$goods_info['comment_point'] = $comment_info['point'];
			$goods_info['comment_num'] = $comment_info['numbers'];
		}
